# REQUIRES: water_python
# RUN: env PYTHONPATH=%py_pkg_root:%PYTHONPATH %python %s | FileCheck %s

from water_mlir import ir
from water_mlir.sympy_to_affine_converter import convert_sympy_to_affine_map

import sympy


def test_in_context(f):
    with ir.Context(), ir.Location.unknown():
        f()
    return f


@test_in_context
def test_basic_conversion():
    symbols = [sympy.Symbol("x"), sympy.Symbol("y"), sympy.Symbol("z")]

    expr = sympy.sympify("x + 2*y + 3 + z")
    result = convert_sympy_to_affine_map(expr, symbols)
    # CHECK: ()[s0, s1, s2] -> (s0 + s2 + s1 * 2 + 3)
    print(result)


@test_in_context
def test_mul_conversion():
    symbols = [sympy.Symbol("x"), sympy.Symbol("y")]

    # CHECK: ()[s0, s1] -> (s0 floordiv 4)
    print(convert_sympy_to_affine_map(sympy.sympify("x/4"), symbols))
    # CHECK: ()[s0, s1] -> (s0 floordiv s1)
    print(convert_sympy_to_affine_map(sympy.sympify("x/y"), symbols))
    # CHECK: ()[s0, s1] -> (3 floordiv s0)
    print(convert_sympy_to_affine_map(sympy.sympify("3/x"), symbols))
    # CHECK: ()[s0, s1] -> ((s1 * 2) floordiv 3)
    print(convert_sympy_to_affine_map(sympy.sympify("y*(2/3)"), symbols))
    # CHECK: Warning: Converting y^(-1) to ceil(1/y)
    # CHECK: ()[s0, s1] -> (1 ceildiv s1)
    print(convert_sympy_to_affine_map(sympy.sympify("1/y"), symbols))
    # CHECK: ()[s0, s1] -> ((s0 + 5) floordiv (s0 + s1))
    print(convert_sympy_to_affine_map(sympy.sympify("(x+5)/(y+x)"), symbols))


@test_in_context
def test_rational_conversion():
    symbols = []

    expr = sympy.sympify("1/2")
    # CHECK: Warning: Rounding rational number 1/2 down
    # CHECK: () -> (0)
    print(convert_sympy_to_affine_map(expr, symbols))

    expr = sympy.sympify("5/2")
    # CHECK: Warning: Rounding rational number 5/2 down
    # CHECK: () -> (2)
    print(convert_sympy_to_affine_map(expr, symbols))


@test_in_context
def test_floor_ceil_conversion():
    symbols = [sympy.Symbol("x"), sympy.Symbol("y"), sympy.Symbol("z")]

    # CHECK: ()[s0, s1, s2] -> ((s0 * 3) floordiv 2)
    print(convert_sympy_to_affine_map(sympy.sympify("floor(3*x/2)"), symbols))
    # CHECK: ()[s0, s1, s2] -> (s1 ceildiv 5)
    print(convert_sympy_to_affine_map(sympy.sympify("ceiling(y/5)"), symbols))
    # CHECK: ()[s0, s1, s2] -> (5)
    print(convert_sympy_to_affine_map(sympy.sympify("floor(5)"), symbols))
    # CHECK: ()[s0, s1, s2] -> (7)
    print(convert_sympy_to_affine_map(sympy.sympify("ceiling(7)"), symbols))
    # CHECK: ()[s0, s1, s2] -> (s2)
    print(convert_sympy_to_affine_map(sympy.sympify("floor(z)"), symbols))
    # CHECK: ()[s0, s1, s2] -> (s1 floordiv s2)
    print(convert_sympy_to_affine_map(sympy.sympify("floor(y/z)"), symbols))
    # CHECK: ()[s0, s1, s2] -> (s0 ceildiv s1)
    print(convert_sympy_to_affine_map(sympy.sympify("ceiling(x/y)"), symbols))
    # CHECK: ()[s0, s1, s2] -> ((s0 + 3) floordiv (s1 + s2))
    print(convert_sympy_to_affine_map(sympy.sympify("floor((3+x)/(y+z))"), symbols))
    # CHECK: ()[s0, s1, s2] -> ((s0 + 2) ceildiv s1)
    print(convert_sympy_to_affine_map(sympy.sympify("ceiling((x+2)/y)"), symbols))


@test_in_context
def test_mod_conversion():
    symbols = [sympy.Symbol("x"), sympy.Symbol("y")]

    # CHECK: ()[s0, s1] -> (s0 mod s1)
    print(convert_sympy_to_affine_map(sympy.sympify("Mod(x, y)"), symbols))
    # CHECK: ()[s0, s1] -> ((s1 floordiv 5) mod 2)
    print(convert_sympy_to_affine_map(sympy.sympify("Mod(y/5, 2)"), symbols))
    # CHECK: ()[s0, s1] -> (3 mod (s0 + 4))
    print(convert_sympy_to_affine_map(sympy.sympify("Mod(3, x+4)"), symbols))
