...
func.func @allocate() -> !wave.tensor<[@M, @N] of bf16, <shared>> {
  // CHECK: wave.allocate
  %parent = wave.allocate { distributed_shape = #wave.distributed_shape<[BLOCK_M, BLOCK_K] -> (BLOCK_M, BLOCK_K + 4)>, logical = !wave.tensor<[@M,@N] of bf16, <shared>> }
    : !wave.tensor<[@M, @N] of bf16, <shared>>

  %buf = wave.allocate in %parent : !wave.tensor<[@M, @N] of bf16, <shared>>
    { distributed_shape = #wave.distributed_shape<[BLOCK_M, BLOCK_K] -> (BLOCK_M, BLOCK_K + 4)>, offset = 64 , logical = !wave.tensor<[@M,@N] of bf16, <shared>>}
    : !wave.tensor<[@M, @N] of bf16, <shared>>
