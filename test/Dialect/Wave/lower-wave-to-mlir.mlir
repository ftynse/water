...
func.func @lower_register_with_unrealized_cast() {
  // CHECK-NOT: wave.register
  // CHECK:     %[[CST:.*]] = arith.constant dense<0.000000e+00> : vector<9x9xf32>
  // CHECK:     %[[CAST:.*]] = builtin.unrealized_conversion_cast %[[CST]] : vector<9x9xf32> to !wave.tensor<[@Y, @Z] of f32>
  // CHECK:     "test.foo"(%[[CAST]])
  %cst = arith.constant 0.0 : f32
  %0 = wave.register %cst : !wave.tensor<[@Y, @Z] of f32>
  "test.foo"(%0) : (!wave.tensor<[@Y, @Z] of f32>) -> ()
  return
}
