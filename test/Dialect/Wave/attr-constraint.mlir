...
// CHECK: #wave.workgroup_constraint<dim = [M] -> (M), tile_size = [BLOCK_M] -> (BLOCK_M floordiv 4), workgroup_dim = 0>
...
// CHECK: #wave.workgroup_constraint<dim = [M] -> (M), tile_size = [BLOCK_M] -> (BLOCK_M floordiv 4), workgroup_dim = 0, iters = [M] -> (M), per_device_dim = [DEVICE_M] -> (DEVICE_M)>
...
// CHECK: #wave.tiling_constraint<dim = [K] -> (K)>
func.func private @test_tiling1() attributes { wave.constraints = [#wave.tiling_constraint<dim = [K] -> (K)>] }


// CHECK: #wave.tiling_constraint<dim = [K] -> (K), tile_size = [BLOCK_K] -> (BLOCK_K),
...
// CHECK: #wave.wave_constraint<dim = [K] -> (K), tile_size = [BLOCK_K] -> (BLOCK_K floordiv 4)>
...
// CHECK: #wave.wave_constraint<dim = [K] -> (K), tile_size = [BLOCK_K] -> (BLOCK_K floordiv 4), wave_id = [THREAD_0] -> (THREAD_0 floordiv 64),
// CHECK:   wg_constraint = <dim = [M] -> (M), tile_size = [BLOCK_M] -> (BLOCK_M floordiv 4), workgroup_dim = 0>
...
// CHECK: #wave.device_constraint<dim = [M] -> (M), tile_size = [DEVICE_M] -> (DEVICE_M), device_dim = 0>
#dv_constraint = #wave.device_constraint<dim = [M] -> (M), tile_size = [DEVICE_M] -> (DEVICE_M), device_dim = 0>
